...
class TrafficGenerator:
    ...
    def generate_routefile(self, seed):
        ...
        # 포아송 분포를 이용하여 자동차가 생성되는 시간을 랜덤으로 생성
        # 첫 번째 인자는 분포의 평균값(lambda), 두 번째 인자는 생성하려는 난수의 개수
        timings = np.random.poisson(self._max_steps, self._n_cars_generated)
        # 생선된 난수들을 오름차순으로 정렬하여 자동차들이 생성되는 시간으로 사용
        ...
        with open("routes.rou.xml", "w") as routes:
            print("""<routes>
            <vType accel="1.0" decel="4.5" id="standard_car" length="5.0" minGap="2.5" maxSpeed="25" allowLaneChange='false' sigma="0.5" />

            <route id="W_N" edges="WtoC CtoN"/>
            <route id="W_E" edges="WtoC CtoE"/>
            <route id="W_S" edges="WtoC CtoS"/>
            <route id="N_W" edges="NtoC CtoW"/>
            <route id="N_E" edges="NtoC CtoE"/>
            <route id="N_S" edges="NtoC CtoS"/>
            <route id="E_W" edges="EtoC CtoW"/>
            <route id="E_N" edges="EtoC CtoN"/>
            <route id="E_S" edges="EtoC CtoS"/>
            <route id="S_W" edges="StoC CtoW"/>
            <route id="S_N" edges="StoC CtoN"/>
            <route id="S_E" edges="StoC CtoE"/>""", file=routes)
