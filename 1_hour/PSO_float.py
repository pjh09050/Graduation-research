...
class Particle:
    def __init__(self, bounds, max_iter):
        ...
        for i in range(len(bounds)):
            self.position.append(round(random.uniform(bounds[i][0], bounds[i][1]),1))
        ...
        while True:
            x1_sum = round(sum(self.position[:5]),1)
            x2_sum = round(sum(self.position[5:10]),1)
            x3_sum = round(sum(self.position[10:]),1)
            if x1_sum == 165.0 and x2_sum == 165.0 and x3_sum == 165.0:
                break

            if x1_sum > 165:
                sort_list = sorted(self.position[:5], reverse=True)
                for i in range(len(sort_list)):
                    if round(sum(self.position[:5]),1) == 165.0:
                        break
                    largest_value_idx = self.position.index(sort_list[i])
                    self.position[largest_value_idx] -= 0.1
                    self.position[largest_value_idx] = round(self.position[largest_value_idx], 1)
            else:
                ...
                for i in range(len(sort_list)):
                    ...
                    self.position[smallest_value_idx] += 0.1
                    self.position[smallest_value_idx] = round(self.position[smallest_value_idx], 1)

            if x2_sum > 165:
                sort_list = sorted(self.position[5:10], reverse=True)
                for i in range(len(sort_list)):
                    if round(sum(self.position[5:10]),1) == 165.0:
                        break
            ...
            if x3_sum > 165:
                ...
                for i in range(len(sort_list)):
                    if round(sum(self.position[10:]),1) == 165.0:
                        break
    ...
    def update_velocity(self, global_best_position):
        ...
        self.velocity = [round(v,1) for v in self.velocity]
        print('수정된 velocity', self.velocity)
        ...
        while True:
            ...
            if x3_sum > 165:
                ...
                for i in range(len(sort_list)):
                    if round(sum(self.position[10:]),1) == 165.0:
                        break
                    largest_value_idx = self.position.index(sort_list[i], 10, 15)
                    self.position[largest_value_idx] -= 0.1
                    self.position[largest_value_idx] = round(self.position[largest_value_idx], 1)
            else:
                sort_list = sorted(self.position[10:], reverse=False)
                for i in range(len(sort_list)):
                    if round(sum(self.position[10:]),1) == 165.0:
                        break
                    smallest_value_idx = self.position.index(sort_list[i], 10, 15)
                    self.position[smallest_value_idx] += 0.1
                    self.position[smallest_value_idx] = round(self.position[smallest_value_idx], 1)
