...
class Particle:
    def __init__(self, bounds, max_iter):
        ...
        x1_sum = sum(self.position[:4])
        x2_sum = sum(self.position[4:8])
        x3_sum = sum(self.position[8:])
        print(x1_sum,x2_sum,x3_sum)

        x1 = self.position[:4]
        x2 = self.position[4:8]
        x3 = self.position[8:]

        if x1_sum != 165:
            x1.insert(1, 165-x1_sum)
        
        if x2_sum != 165:
            x2.insert(1, 165-x2_sum)

        if x3_sum != 165:
            x3.insert(2, 165-x3_sum)
    ...
    def update_position(self, bounds):
        for i in range(len(self.position)):
            ...
            if sum(x1) != 165:
                x1.insert(1, 165-sum(x1))

            if sum(x2) != 165:
                x2.insert(1, 165-sum(x2))

            if sum(x3) != 165:
                x3.insert(2, 165-sum(x3))
